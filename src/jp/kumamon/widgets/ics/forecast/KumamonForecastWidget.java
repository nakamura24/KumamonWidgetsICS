...
public class KumamonForecastWidget extends WidgetBase {
	...
	public static final String LOCATEID = "ics.forecast.LocateId";
	public static final String LASTLOCATEID = "ics.forecast.LastLocateId";
	public static final String LASTUPDATE = "ics.forecast.LastUpdate";
	public static final String APPWIDGET_ALARM = "jp.kumamon.widgets.ics.forecast.APPWIDGET_ALARM";
	...
	public void onUpdate(Context context, AppWidgetManager appWidgetManager,
		...
		try {
			for (int i = 0; i < appWidgetIds.length; i++) {
				...
				// Forecast Get
				getForecast(mContext, appWidgetIds[i]);
		...
		try {
			if (ForecastWidgetConfigure.CONFIG_DONE.equals(intent.getAction())) {
				...
				if (extras != null) {
					...
					getForecast(context, appWidgetId);
				}
			}
		...
		public void onLocationChanged(Location location) {
			...
			if (location != null) {
				try {
					...
					StaticHash hash = new StaticHash(mContext);
					for (int i = 0; i < hash.keys(LOCATEID).size(); i++) {
						getForecast(mContext,
								Integer.parseInt(hash.keys(LOCATEID).get(i)));
					}
				} catch (Exception e) {
					ExceptionLog.Log(TAG, e);
				}
	...
	private static CityEntries mCityEntries = null;
	...
	private static void getForecast(Context context, int appWidgetId) {
			...
			if (hash.contains(LASTLOCATEID, String.valueOf(appWidgetId))
				...
				if (hash.contains(LASTUPDATE, String.valueOf(appWidgetId))) {
					...
					if (lastUpdate.get(Calendar.DAY_OF_YEAR) == nowDate
							...
							&& lastUpdate.get(Calendar.HOUR_OF_DAY) >= 6
					...
					if (lastUpdate.get(Calendar.DAY_OF_YEAR) == nowDate
							.get(Calendar.DAY_OF_YEAR)
							&& lastUpdate.get(Calendar.HOUR_OF_DAY) < 6
							&& nowDate.get(Calendar.HOUR_OF_DAY) < 6)
						return;
				}
			}
			// システムから接続情報をとってくる
